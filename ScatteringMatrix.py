...
#see Optical.py
#we use the notation N=n+ik n,k>=0
#First layer is the transparent (k=0) semi-infinite medium where 
#the light comes from, the incidence angle Fi is measured here.
...
def AdaptData(structure,lam,Fi):
    for i in range(len(structure)):
        structure[i][1]=array(structure[i][1])
    return structure,array(lam),structure[0][1]*sin(Fi)
...
def ComputeRo(structure,lam,Fi):#
    ...
    ro=rp/rs
    delta=-log(ro).imag#because (n+ik) notation we must revert the sign of delta
    psi=arctan(abs(ro))
    N=structure[0][1].real
    den=(1+sin(2*psi)*cos(delta))**2
    e1=N**2*sin(Fi)**2*(tan(Fi)**2*(cos(2*psi)**2-sin(2*psi)**2*sin(delta)**2)/den+1)
    e2=N**2*sin(Fi)**2*tan(Fi)**2*sin(4*psi)*sin(delta)/den
    e=e1+e2*1j
    n=e**(0.5)
    test = delta < 0#convert -180 to 180 deg to 0 to 360 deg form
    delta = delta + test*2*pi
    return psi*180/pi,delta*180/pi,e1,e2,n.real,n.imag
...
#treat the multilayer as coherent and calculates the scattering matrix, T and R
    ...
    for i in range(len(structure)-1):
        ...
        al=W((2*pi/lam)*(2*N1*CosFi1)*rough)
        be=W((2*pi/lam)*(2*N2*CosFi2)*rough)
        ga=W((2*pi/lam)*(N1*CosFi1-N2*CosFi2)*rough)
        gat=ga*t
        intmatrix.append(array([[1/gat,be*r/gat],[al*r/gat,(ga**2*(1-r**2)+al*be*r**2)/gat]]))
    ...
    for i in range(1,len(structure)-1):
        ...
        phamatrix.append(array([[exp(-1j*beta),lam*0],[lam*0,exp(1j*beta)]]))
    ...
    if pol == "p":
        T=abs(T)**2*(conjugate(N2)*CosFi(NSinFi,N2)).real/((N1*CosFi(NSinFi,N1))).real#Transmittance
    else:
    ...
    for i in range(len(structure)):
        if structure[i][2]==True or i==(len(structure)-1):
            ...
            if i != (len(structure)-1):
                ...
                d=structure[i][0]
                beta=PhaseShift(d,N,NSinFi,lam)
                GS.append(array([[abs(exp(-1j*beta))**2,lam*0],[lam*0,abs(exp(+1j*beta))**2]]))
    return GS
...
#for a mixed coherent/incoherent multilayer return the modified scattering matrix, T and R
    ...
    N2=structure[len(structure)-1][1]
    if pol == "p":
        T=T*(conjugate(N2)*CosFi(NSinFi,N2)).real/((conjugate(N1)*CosFi(NSinFi,N1))).real
    else:
        T=T*(N2*CosFi(NSinFi,N2)).real/((N1*CosFi(NSinFi,N1))).real
    return S,T,R

def PhaseShift(d,N,NSinFi,lam):#compute phase shift
    beta = 2*pi*d*N*CosFi(NSinFi,N)/lam
...
#compute Energy Flux in layer j at relative position x (0<=x<=1)
    ...
    U=[]
    M=array([1/S[0][0],0*lam])
    U.append([M[0],M[1]])
    for i in range(len(GS)-1,-1,-1):
        M=MATRIXMULTVECT(GS[i],M)
        U.append([M[0],M[1]])
    U.reverse()
    #find in wich incoherent matrix is layer j => c
    c=0
    test=False
    for i in range(j+1):
    ...
    if structure[j][2]==True:#layer j is incoherent
        ...
        L=(array([[abs(exp(-1j*beta))**2,lam*0],[lam*0,abs(exp(+1j*beta))**2]]))
        Ux=MATRIXMULTVECT(L,U[c+1])
        if pol == "p":
            EFlux=(Ux[0]-Ux[1])*(conjugate(N)*cosFi).real/((conjugate(N0)*cosFi0)).real
        else:
        ...
        L=(array([[exp(-1j*beta),lam*0],[lam*0,exp(+1j*beta)]]))
        #Light flux coming from left
        E=MATRIXMULTVECT(Sj,array([1/S1[0][0],lam*0])*(U[c][0])**(0.5))
        Ex=MATRIXMULTVECT(L,E)
        EFluxl=Poynting(Ex,N,N0,cosFi,cosFi0,pol)
        #Light flux coming from rigth
        E=MATRIXMULTVECT(Sj,array([-S1[0][1]/S1[0][0],lam/lam])*(U[c+1][1])**(0.5))        
        Ex=MATRIXMULTVECT(L,E)
...
def Poynting(Ex,N,N0,cosFi,cosFi0,pol):#compute energy flux
    if pol == "p":
        Flux=(abs(Ex[0])**2-abs(Ex[1])**2)*(conjugate(N)*cosFi).real-2*((Ex[0]*conjugate(Ex[1])).imag*(conjugate(N)*cosFi).imag)
    else:
        Flux=(abs(Ex[0])**2-abs(Ex[1])**2)*(N*cosFi).real-2*((Ex[0]*conjugate(Ex[1])).imag*(N*cosFi).imag)
