...
scan_rate = 0.33 #every x seconds


#print (N_c_high[::25])

#Layer = [0=thickness in nm, 1=nk, 2=incoherent, 3=roughnes in Angstrom, 4=deposition rate in nm/s, 5=rate variation in nm/s, 6=Ihomogeneity, 7=n_variation]
...
layer_high = [qwot2, N_c_high, False, 0, 0.6, 0.06, [True, 0.93, 0, 1.0, 4.0], 'Layer_high']
layer_low = [qwot1, N_c_low, False, 0, 1.0, 0.1, [True, 0.93, 0, 5.0, 5.0],'Layer_low']
...
layer_list_reference = [layer_high, layer_low, layer_high, layer_low, layer_high, layer_low]
...
for element in layer_list_reference:
    if element[6][0] == True:
        ...
        temp_list = []
        thick = element[0]
        #print ('length sub layer list', len(sub_layer_list))
        for idx, sub_layer in enumerate(sub_layer_list):
            #print ('sublayer thick', sub_layer[0])
            if sub_layer[0] <= thick:
                temp_list.append(sub_layer)
                thick = thick - sub_layer[0]           
                #print (idx, thick)
            else:
                if thick == 0:
                    break
                    ...
                    sub_layer[0] = thick
                    temp_list.append(sub_layer)
                    #print ('last thick ', thick)
            
        layer_list_ref_physical.append(temp_list)
    else:
...
for idx, element in enumerate(layer_list_physical):
    print (idx)
    temp_list = []
    thick = layer_list_reference[idx][0]
    #print ('length sub layer list', len(sub_layer_list))
    for ix, sub_layer in enumerate(element):
        #print ('sublayer thick', sub_layer[0])
        if sub_layer[0] <= thick:
            temp_list.append(sub_layer)
            thick = thick - sub_layer[0]           
...
def sub_thickness_calculator(sub_layer_list):
    thick = 0
    for idx,element in enumerate(sub_layer_list):
        thick = thick + element[0]
    return thick    

#for idx,element in enumerate(layer_list_ref_physical):
#    print ('Length layer_System', len(layer_list_ref_physical))
#    for sub_layer in element:
#        print ('Length Sublayer',len(element))
#        print ('Sublayer thickness ', sub_layer[0])
...
gs = GridSpec(nrows=2, ncols=1, height_ratios=[2, 1], left=0.125, right=0.95, bottom=0.05, top=0.95, figure=fig)
#fig, axs = plt.subplots(2, 1)

#fig.suptitle('BBM Monitoring Simulations')

ax1 = plt.subplot(gs[0])
ax2 = plt.subplot(gs[1])

line1, = ax1.plot(wavl, RT_start[spc], label='Monitoring Singal', color='mediumspringgreen')
line2, = ax1.plot(wavl, RT_start[spc], label='Preview', linestyle='--', color='darkgoldenrod')
line4, = ax1.plot(wavl, RT_start[spc], label='Reference', linestyle='--', color='slategrey')
line3, = ax1.plot(wavl, RT_start[spc], label='Opt', linestyle='-.', color='darkcyan')
ax1.set_title('BBM Monitoring Curve')
ax1.set_xlabel('wavelength')
ax1.set_ylabel('T&R')
ax1.legend(loc='upper right')
ax1.grid(True)

deposition_rate_list = []
deposition_new_rate_list = []
x_rate_time = []

line5, = ax2.plot(x_rate_time, deposition_rate_list, label='deposition_rate', color='mediumspringgreen')
line6, = ax2.plot(x_rate_time, deposition_new_rate_list, label='monitoring_rate', linestyle='--', color='darkgoldenrod')
...
preview_system =  environment_2[::-1]
...
for idx, element in enumerate(layer_list_reference):
    ...
    #print ('top_level ', idx)
    for sub_l in layer_list_ref_physical[idx]:
        preview_system.append(sub_l)
    preview_system.append(environment_1)
    length_sublayers = layer_list_ref_physical[idx]
    ...
    RT_preview = ScatteringMatrix.ComputeRT(preview_system[::-1],wavl,AOI)
    ...
    for ix, rate in enumerate(rate_noise_list[idx]):
        ...
        #if new_thickness + new_rate_average * shutter_delay >= target_thickness:
        ...
        deposition_time = deposition_time + scan_rate
        deposition_rate_list.append(rate)
        deposition_new_rate_list.append(new_rate)
        x_rate_time.append(deposition_time)

        line5.set_xdata(x_rate_time)
        line5.set_ydata(deposition_rate_list)
        line6.set_xdata(x_rate_time)
        line6.set_ydata(deposition_new_rate_list)
        #ax2.autoscale()
        ax2.set_xlim(0.0,max(x_rate_time)+1)
        ax2.set_ylim(0.0,max(deposition_new_rate_list)+0.01)
        
        line1.set_ydata(spectral_data)
        
        line3.set_ydata(opt_spectral_data[spc])
        ax1.set_ylim(RT[spc].min()-0.05,RT[spc].max()+0.05)
        #layer_system.append(environment_1)
        plt.pause(0.001)    
    line2.set_ydata(RT[spc])
    preview_system = layer_system[:-1]
    layer_system = layer_system[:-1]
